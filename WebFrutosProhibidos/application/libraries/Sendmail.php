    ...
    public function success_sendmail($email){
        ...
        try {
            ...
            foreach($mail as $key => $value){
                    //$phpmailer->AddAddress($value, $key);
                $phpmailer->SetFrom($this->email, $this->name);
                echo '<script>console.log("prueba1")</script>';
                $phpmailer->AddAddress($value, $key);
                echo '<script>console.log("prueba2")</script>';
                $phpmailer->Subject = $this->subject;
                echo '<script>console.log("prueba3")</script>';
                $phpmailer->AltBody = 'Si ve este mensaje, por favor use un HTML compatible.';
                echo '<script>console.log("prueba4")</script>';
                $phpmailer->MsgHTML($body);
                echo '<script>console.log("prueba5")</script>';
                $phpmailer->CharSet = 'UTF-8';
                echo '<script>console.log("prueba6")</script>';
                $phpmailer->Send();
                echo '<script>console.log("prueba7")</script>';
                $phpmailer->ClearAllRecipients();
                echo '<script>console.log("prueba8")</script>';
